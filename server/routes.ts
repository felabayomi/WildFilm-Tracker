...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/films/discover", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const keywords = req.query.keywords || "nature|wildlife|animal";
      
      const url = `${TMDB_BASE_URL}/discover/movie?api_key=${apiKey}&with_genres=${DOCUMENTARY_GENRE_ID}&sort_by=popularity.desc&page=${page}&vote_count.gte=50`;
